"""
+------------------------------------------------------------------+
|  PDF Formulario Agentes / Directivos  ->  Excel   (v2)           |
|  Grupo ATU (c) 2026                                              |
|  Hecho por RaulRDA.com, Pablo Alvarez y Pelayo Fernandez         |
+------------------------------------------------------------------+
...
PLANTILLAS = {
    TIPO_DIRECTIVOS: ("_plantillas/formulario_directivos.xlsm", "Formulario Directivos"),
    TIPO_AGENTES_1:  ("_plantillas/formulario_agentes1.xlsm",   "formulario agentes"),
    TIPO_AGENTES_2:  ("_plantillas/formulario_agentes2.xlsm",   "formulario agentes"),
}
...
# ==========================================================
# MAPEOS  campo -> celda Excel  (columna B, notacion "B6")
# ==========================================================
MAPEO_DIRECTIVOS = {
    "PRIMER APELLIDO":            "B6",
    "SEGUNDO APELLIDO":           "B7",
...
# Campos de empresa que se vacian si el alumno esta desempleado
CAMPOS_EMPRESA = [
...
# Campos reales que validan que la extraccion tuvo exito
CAMPOS_REALES = [
    "PRIMER APELLIDO", "SEGUNDO APELLIDO", "NOMBRE",
    "EMAIL", "TELEFONO", "NIF EMPRESA", "NOMBRE EMPRESA", "DIRECCION",
]

# Palabras de inicio de campo (para cortar extraccion multilinea)
ENCABEZADOS_STOP = {
...
def normalizar(texto):
    """Mayusculas, sin tildes, espacios colapsados. Clave interna."""
    if not texto:
        return ""
    trans = str.maketrans(
        "\xe1\xe9\xed\xf3\xfa\xe0\xe8\xec\xf2\xf9"
        "\xe2\xea\xee\xf4\xfb\xe4\xeb\xef\xf6\xfc"
        "\xc1\xc9\xcd\xd3\xda\xc0\xc8\xcc\xd2\xd9"
        "\xc2\xca\xce\xd4\xdb\xc4\xcb\xcf\xd6\xdc"
        "\xf1\xd1",
        "aeiouaeiouaeiouaeiou"
        "AEIOUAEIOUAEIOUAEIOu"
        "nN"
    )
    resultado = texto.upper().translate(trans).replace("\xba", "")
...
def postprocesar_campos(datos, tipo):
    ...
    # 6. Ambito rural puede traer texto largo
    ...
    # 9. Correccion tipo/num documento
...
# ==========================================================
# DETECCION AUTOMATICA DEL TIPO DE FORMULARIO
# ==========================================================
def detectar_tipo_pdf(lineas):
    cabecera = " ".join(lineas[:8]).upper()
    texto    = " ".join(lineas).upper()

    if "AGENTES DEL CAMBIO" in cabecera:
        if "1-249" in texto or "1 - 249" in texto:
            return TIPO_AGENTES_2, 0.95
        return TIPO_AGENTES_1, 0.90

    if "GENERACION DIGITAL" in normalizar(cabecera) or "DIRECTIV" in cabecera:
        return TIPO_DIRECTIVOS, 0.90

    puntos = {TIPO_DIRECTIVOS: 0, TIPO_AGENTES_1: 0, TIPO_AGENTES_2: 0}
    if "TITULACION" in texto or "RELACION CON LA EMPRESA" in texto:
        puntos[TIPO_DIRECTIVOS] += 2
    if "LINKEDIN" in texto:
        puntos[TIPO_AGENTES_1] += 1
        puntos[TIPO_AGENTES_2] += 1
    if "AGENTES" in texto:
        puntos[TIPO_AGENTES_1] += 1
        puntos[TIPO_AGENTES_2] += 1
    if "1-249" in texto or "1 - 249" in texto:
        puntos[TIPO_AGENTES_2] += 2
    if "10-249" in texto or "10 - 249" in texto:
        puntos[TIPO_AGENTES_1] += 2

    mejor = max(puntos, key=puntos.get)
    total = sum(puntos.values())
    confianza = (puntos[mejor] / total) if total > 0 else 0.0
    return mejor, min(confianza, 0.75)


# ==========================================================
# LECTURA DE LINEAS DEL PDF
# ==========================================================
...
def _mapear_campo_tabla(etiq, valor, datos, tipo):
    ...
    elif ("NUM" in en or "N\xba" in e or "N\xb0" in e) and "DOCUMENTO" in e:
    ...
    elif "DIRECCION" in e and "EMPRESA" in e:
...
# Mapa clave-normalizada -> nombre-canonico (construido una sola vez)
...
def extraer_campos_pdf(ruta_pdf, tipo, log, lineas=None):
    ...
    try:
        ...
        log.info("  [tablas] {} campos".format(len(datos_tablas)))
    except Exception as e:
        log.warning("  [tablas] fallo: {}".format(e))
    ...
    try:
        ...
        if _coord_es_util(datos_coord):
            ...
            log.info("  [coordenadas] {} campos adicionales".format(anadidos))
        else:
            log.warning("  [coordenadas] resultado poco fiable, omitido")
    except Exception as e:
        log.warning("  [coordenadas] fallo: {}".format(e))
    ...
    try:
        ...
        log.info("  [lineas] {} campos adicionales".format(len(datos_lineas)))
    except Exception as e:
        log.warning("  [lineas] fallo: {}".format(e))

    log.info("  Total campos extraidos: {}".format(len(datos)))
    return datos
...
def validar_datos(datos):
    return any(c in datos for c in CAMPOS_REALES)
    ...
    try:
        ...
        for hoja in hojas:
            if nombre_hoja_buscar.lower() in hoja.lower():
                return hoja
        log.error("Hoja '{}' no encontrada".format(nombre_hoja_buscar))
        return None
    except Exception as e:
        log.error("Error al leer Excel: {}".format(e))
        return None
...
def rellenar_excel(datos, tipo, plantilla, salida, nombre_hoja, log):
    ...
    if nombre_hoja not in wb.sheetnames:
        log.error("Hoja '{}' no encontrada".format(nombre_hoja))
        return 0
    ws = wb[nombre_hoja]
    celdas_ok = 0
...
def obtener_nombre_archivo(datos):
    ...
    base     = re.sub(r'[\\/*?:"<>|]', "", base)
    return "Formulario_{}.xlsm".format(base)
...
def guardar_log_errores(carpeta, errores):
    ...
    ruta = carpeta / "log_errores_{}.txt".format(ts)
    with open(ruta, "w", encoding="utf-8") as f:
        f.write("LOG DE ERRORES - Conversor PDF -> Excel\n")
        f.write("Generado: {}\n".format(datetime.now().strftime("%Y-%m-%d %H:%M:%S")))
        f.write("Total errores: {}\n\n".format(len(errores)))
...
# ==========================================================
# UI - VENTANA DE LOG CON BARRA DE PROGRESO
# ==========================================================
class VentanaLog:
    def __init__(self, titulo="Procesando", total=0):
        ...
        self.ventana.title("Grupo ATU - {}".format(titulo))
        ...
        self.lbl_estado = tk.Label(
            header, text="Procesando...", font=("Arial", 9),
            bg=ATU_NAVY, fg=ATU_ORANGE
        )
        ...
        self.lbl_prog = tk.Label(
            prog_frame, text="0 / {}".format(total),
        ...
        self.texto.tag_config("TITLE", foreground=ATU_ORANGE,
        ...
        tk.Label(
            bottom,
            text="Hecho por RaulRDA.com  |  Pablo Alvarez  |  Pelayo Fernandez",
            font=("Arial", 7), bg=ATU_NAVY, fg=ATU_MID
        ).pack(side=tk.LEFT)
    ...
    def info(self, msg):    self._escribir("  " + msg, "INFO")
    def ok(self, msg):      self._escribir("+ " + msg, "OK")
    def error(self, msg):   self._escribir("x " + msg, "ERROR")
    def warning(self, msg): self._escribir("! " + msg, "WARN")
    ...
    def avanzar(self):
        ...
        self.lbl_prog.config(text="{} / {}".format(self._actual, self.total))
        self.ventana.update()

    def completado(self, fallidos):
        ...
        if fallidos:
            self._escribir(
                "\n  No se convirtieron {} archivo(s):".format(len(fallidos)), "WARN"
            )
            for f in fallidos:
                self._escribir("    - " + f, "ERROR")
        self.btn_cerrar.config(
            state=tk.NORMAL, bg=ATU_ORANGE, fg=ATU_WHITE, font=("Arial", 9, "bold")
        )
...
# ==========================================================
# DIALOGO DE CONFIRMACION DE TIPO
# ==========================================================
def pedir_tipo_manual(tipo_detectado):
    ventana = tk.Toplevel()
    ventana.title("Grupo ATU - Confirmar tipo de formulario")
    ...
    header = tk.Frame(ventana, bg=ATU_NAVY, pady=14)
    header.pack(fill=tk.X)
    ...
    body = tk.Frame(ventana, bg=ATU_LIGHT, pady=16)
    body.pack(fill=tk.BOTH, expand=True)
    tk.Label(
        body,
        text="No se pudo detectar el tipo con certeza.\nConfirma o elige el tipo correcto:",
        font=("Arial", 10), bg=ATU_LIGHT, fg=ATU_TEXT, justify=tk.CENTER
    ).pack(pady=(0, 10))

    tipo_var = tk.StringVar(value=tipo_detectado)
    combo = ttk.Combobox(
        body, textvariable=tipo_var,
    ...
    btn_f = tk.Frame(body, bg=ATU_LIGHT)
    btn_f.pack(pady=12)
    tk.Button(
        btn_f, text="  Aceptar  ", command=aceptar,
        bg=ATU_ORANGE, fg=ATU_WHITE, font=("Arial", 10, "bold"),
        relief=tk.FLAT, cursor="hand2", padx=10, pady=6
    ).pack(side=tk.LEFT, padx=8)
    tk.Button(
        btn_f, text="  Cancelar  ", command=cancelar,
...
def main():
    ...
    pdfs = filedialog.askopenfilenames(
        title="Selecciona los formularios PDF",
        filetypes=[("PDF", "*.pdf")]
    )
    if not pdfs:
        root.destroy()
    ...
    log = VentanaLog("Procesando formularios", total=len(pdfs))
    log.info("PDFs seleccionados : {}".format(len(pdfs)))
    log.info("Destino            : {}".format(carpeta_salida))
    ...
    for i, pdf in enumerate(pdfs, 1):
        pdf_path = Path(pdf)
        log.titulo("[{}/{}] {}".format(i, len(pdfs), pdf_path.name))

        try:
            try:
                lineas = leer_lineas_pdf(str(pdf_path))
            except Exception as e:
                raise ValueError("PDF ilegible: {}".format(e))

            tipo, confianza = detectar_tipo_pdf(lineas)
            log.info("  Tipo detectado: {}  (confianza {:.0%})".format(tipo, confianza))

            if confianza < 0.60:
                log.warning("  Confianza baja - solicitando confirmacion")
                tipo_conf = pedir_tipo_manual(tipo)
                if tipo_conf is None:
                    raise ValueError("Cancelado por el usuario en seleccion de tipo")
                tipo = tipo_conf
                log.info("  Tipo confirmado: {}".format(tipo))

            archivo_plantilla, nombre_hoja_buscar = PLANTILLAS[tipo]
            ruta_plantilla = BASE_DIR / archivo_plantilla
            if not ruta_plantilla.exists():
                raise FileNotFoundError("Plantilla no encontrada: {}".format(ruta_plantilla))

            nombre_hoja = verificar_hoja_excel(str(ruta_plantilla), nombre_hoja_buscar, log)
            if not nombre_hoja:
                raise ValueError("Hoja '{}' no encontrada en la plantilla".format(nombre_hoja_buscar))

            datos = extraer_campos_pdf(str(pdf_path), tipo, log, lineas=lineas)

            if not validar_datos(datos):
                raise ValueError(
                    "No se extrajeron campos reales. "
            ...
            while ruta_salida.exists():
                ruta_salida = carpeta_salida / "{}_{}.xlsm".format(
                    ruta_salida.stem, contador
                )
                contador += 1
            ...
            if celdas > 0:
                log.ok("Guardado: {}  ({} celdas)".format(ruta_salida.name, celdas))
                exitosos += 1
            else:
                raise ValueError("No se escribio ninguna celda en el Excel")
    ...
    if fallidos:
        try:
            ...
            if ruta_log:
                log.info("Log de errores guardado: {}".format(ruta_log.name))
        except Exception as e:
            log.warning("No se pudo guardar el log: {}".format(e))

    log.completado([f for f, *_ in fallidos])

    resumen = "Convertidos: {} / {}".format(exitosos, len(pdfs))
    if fallidos:
        resumen += "\n\nNo convertidos ({}):\n".format(len(fallidos))
        resumen += "\n".join("  - " + f for f, *_ in fallidos)
    resumen += "\n\nGuardados en:\n{}".format(carpeta_salida)
    if ruta_log:
        resumen += "\n\nLog de errores:\n{}".format(ruta_log)
